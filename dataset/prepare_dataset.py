...
def get_dataset(
    dataset_name, save_dir, tokenizer, transform, train_batch_size, num_workers
):
    # load dataset
    ...
    image_column = [
        col for col in ["image", "img"] if col in dataset["train"].column_names
    ][0]
    caption_colum = [
        col for col in ["text", "caption"] if col in dataset["train"].column_names
    ][0]
